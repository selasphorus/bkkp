		...
		<?php foreach ($rows as $row): ?>
			<tr class="<?php echo $row['level'] > 0 ? 'child-category' : 'parent-category'; ?>">
				...
				<?php foreach ($row['cols'] as $col): ?>
					...
					//$amount = number_format(abs($col['sum']), 2); // Not currently in use -- maybe build in the option to display either way?
					$amount_rounded = number_format(round(abs($col['sum']), 2));
